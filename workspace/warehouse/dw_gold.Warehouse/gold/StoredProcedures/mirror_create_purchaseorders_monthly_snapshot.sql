CREATE   PROCEDURE [gold].[mirror_create_purchaseorders_monthly_snapshot]
...
BEGIN
    ...
    IF EXISTS (select 1 from [INFORMATION_SCHEMA].[TABLES] WHERE TABLE_TYPE ='BASE TABLE' AND TABLE_SCHEMA ='gold' AND TABLE_NAME ='Mirror_PurchaseOrders_monthly_snapshot')
        DROP TABLE [gold].[Mirror_PurchaseOrders_monthly_snapshot]

    CREATE TABLE [gold].[Mirror_PurchaseOrders_monthly_snapshot] AS
    SELECT
        po.[SupplierID],
        DATEFROMPARTS(YEAR(po.[OrderDate]), MONTH(po.[OrderDate]), 1) AS [OrderDate],
        COUNT(1) AS [POCount]
    FROM
        [lh_silver].[dbo].[silver_mirror_purchasing_purchaseorders] po
    GROUP BY
		po.[SupplierID],
        DATEFROMPARTS(YEAR(po.[OrderDate]), MONTH(po.[OrderDate]), 1);
END